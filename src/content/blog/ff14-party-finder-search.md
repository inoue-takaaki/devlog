...
ここで一瞬、「これって Content（コンテンツ）テーブルに持たせるのかな？」と考えたんですが、すぐに違うと気づきました。VC を使うかどうかは「零式」というコンテンツ自体の性質ではなく、**その募集ごとに決めること**です。なので Content マスタではなく、募集（PartyPost）側に `voiceChat`（あり / なし / どちらでも）という列を1つ足しました。「どのテーブルに列を置くか」は、その情報が"誰に属するか"で決まる——当たり前なんですが、手を動かしていると迷うポイントでした。
...
- VC 有無は「募集ごとの属性」。コンテンツのマスタではなく PartyPost に列を足した（列の置き場所は"誰に属するか"で決める）
