...
次回はいよいよ `packages/shared`（フロントとバックで共有する型の置き場）を作り、Docker で MySQL を立てて、**Drizzle でスキーマを実装**します。ここで、設計編でずっと「概念」だったデータモデルが、初めて**物理的なテーブル定義**として姿を現します。ようやくコードを書けるので、楽しみです。
